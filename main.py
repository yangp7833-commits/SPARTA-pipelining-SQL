...
def main():
    ...
    if args.store: # triggers the store function if flag is detected
        ...
        if args.file_path: 
            ...
            print("No file path provided. Please provide a file path using --file_path.")
            sys.exit()
        ...
        if args.limit: #checks for the limit and makes sure that it is valid
            if (args.limit > 0) and isinstance(args.limit, int):
                ...
                print("Invalid limit value. Please provide a positive integer for limit.")
                sys.exit()
        ...
        if args.query_gene_results: # if the command was to query gene results, queries gene results
            try: # tries in case of error
                ...
                print(f'Error occurred while listing gene results: {e}')
                sys.exit(1)
            sys.exit()
        else:
            try:
                ...
                print(f'Error occurred while listing experimental data: {e}')
                sys.exit(1)
            sys.exit()
        
    if args.delete_gene_results:
        if args.delete_gene_results=='default':
            print("No deletion query provided. Please provide a query to specify which gene results to delete.")
            sys.exit(1)
        try:
            mainproccesses.delete_gene_results(args.delete_gene_results)
        except Exception as e:
            print(f'Error occurred while deleting gene results: {e}')
            sys.exit(1)
        sys.exit()
    if args.delete_experimental_data:
        if args.delete_experimental_data=='default':
            print("No deletion query provided. Please provide a query to specify which experimental data to delete.")
            sys.exit(1)

        try:
            mainproccesses.delete_experiments(args.delete_experimental_data)
        except Exception as e:
            print(f'Error occurred while deleting experiments: {e}')
            sys.exit(1)
        sys.exit()
