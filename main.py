...
def main():
    ...
    parser.add_argument('--query_gene_results', type=str, help='SQL-like query to filter gene results', nargs='?', const="Default")
    parser.add_argument('--query_experimental_data', nargs='?', type=str, help='SQL-like query to filter experimental data', const="Default")
    ...
    if args.store: # triggers the store function if flag is detected
        ...
        if args.query_gene_results: # if the command was to query gene results, queries gene results
            try: # tries in case of error
                mainproccesses.view_gene_results(args.query, sort_by, limit) # passes the arguments on to the main function, which cleans the query and lists the results
            ...
            try:
                ...
                mainproccesses.view_experiments(args.query, limit)
            except Exception as e:
